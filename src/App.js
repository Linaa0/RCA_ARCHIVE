...
const year1Subjects = [
  "Mathematics", "Physics", "Fundamentals of Programming", "Database", "English",
  "Networking", "PHP", "JavaScript", "Embedded Systems", "Web User Interface",
  "Graphical User Interface", "Computer Basics", "Short Courses",
];
    ...
    <div className="recently-added-section">
      <div className="recently-added-inner">
        <div className="recently-added-header">
          ...
          <a href="#subjects" className="view-all-link" style={{ color: "#2563eb", textDecoration: "none", fontSize: "0.85rem", fontWeight: 500 }}>
            View all →
          </a>
        </div>
          ...
          {papers.map((paper) => {
            ...
            return (
              <a key={paper.id} href={`/view/${paper.id}`} className="recent-card">
                <div className="recent-card-icon" style={{ background: typeInfo.bg }}>{typeInfo.label}</div>
                <div className="recent-card-title">{paper.title}</div>
                <div className="recent-card-meta">{typeInfo.label} · Year {paper.year}</div>
  ...
  return (
    <div className="paper-card">
      ...
      <div className="paper-actions">
        <a href={`/view/${paper.id}`} target="_blank" rel="noopener noreferrer" className="view-btn">View</a>
        <a href={`/api/papers/${paper.id}/download`} className="download-btn">Download</a>
      </div>
    </div>
  );
}

function HomeContent({ search, setSearch, onSearch, results, loading, clearSearch, activeYear, setActiveYear, isDarkMode, toggleDarkMode }) {
  return (
    <div className="page-layout-with-sidebar" id="years">
      <Sidebar activeYear={activeYear} onYearChange={setActiveYear} allYears={[1, 2, 3]} isDarkMode={isDarkMode} toggleDarkMode={toggleDarkMode} />
        ...
        {results !== null ? (
          <>
            ...
            <div className="search-results-section">
              ...
              {results.length === 0 ? (
                ...
                <div>{results.map((paper) => <PaperCard key={paper.id} paper={paper} />)}</div>
              )}
            </div>
          </>
        ) : (
          <>
            ...
            <div className="home-after-hero">
              <SubjectsList subjects={allSubjectsByYear[activeYear]} activeYear={activeYear} />
              <RecentlyAdded />
            </div>
          </>
        )}
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("/api/signup", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: form.email, username: form.username, password: form.password, otp: form.otp }),
      });
      const data = await res.json();
  ...
  return (
    <main className="login-page">
      <div className="archive-auth-shell" style={{ justifyContent: "center" }}>
        <section className="auth-panel" style={{ width: "100%", maxWidth: "460px" }}>
          <div className="auth-card">
            <h1 className="form-title">Create student account</h1>
            <p className="form-subtitle">Join RCA Archive to access study materials and past papers.</p>
            ...
            <form className="login-form" onSubmit={handleSubmit}>
              <div className="form-group">
                ...
                <div className="input-shell">
                  <input type="email" value={form.email} onChange={(e) => setForm((p) => ({ ...p, email: e.target.value }))} placeholder="your@email.com" required disabled={otpSent} />
                </div>
              </div>
              <div className="form-group">
                <label className="auth-label active">Username</label>
                <div className="input-shell">
                  <input type="text" value={form.username} onChange={(e) => setForm((p) => ({ ...p, username: e.target.value }))} placeholder="Choose a username" required />
                </div>
              </div>
              <div className="form-group">
              ...
              {otpSent && (
                <div className="form-group">
                  ...
                  <div className="input-shell">
                    <input type="text" value={form.otp} onChange={(e) => setForm((p) => ({ ...p, otp: e.target.value }))} placeholder="Enter code from email" required />
                  </div>
                </div>
              )}
...
function App() {
  ...
  const clearSearch = () => { setResults(null); setSearch(""); };
          ...
          element={
            <PrivateRoute>
              <HomeContent
                ...
                results={results} loading={loading} clearSearch={clearSearch}
